...
const Sidebar = ({ isOpen }) => {
    return (
        <div
            className={`bg-custom-pink text-white h-screen fixed z-50 top-0 left-0 ease-in-out duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full"
                } md:translate-x-0 w-16 md:w-20 p-4 rounded-3xl hover:w-40 transition-width duration-300 shadow-lg`}
        >
            <ul className="flex flex-col items-start ml-2 mt-40 space-y-6 overflow-hidden">
                {/* Sidebar Links */}
